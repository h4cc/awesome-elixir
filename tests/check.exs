...
defmodule Awesome do
  ...
  defp uniq_links([head | tail], linkcount) do
    ...
    cnt =
      case Map.fetch(linkcount, link) do
        :error ->
          1
  ...
  def test_file(file) do
    ...
    [%Earmark.Block.ListItem{blocks: [%Earmark.Block.Para{} | categories]} | tableOfContent] =
      tableOfContent

    [%Earmark.Block.List{blocks: categories}] = categories

    categories =
      for %Earmark.Block.ListItem{blocks: [%Earmark.Block.Para{lines: [name]}]} <- categories do
    ...
    [%Earmark.Block.ListItem{blocks: [%Earmark.Block.Para{} | resources]} | _tableOfContent] =
      tableOfContent

    [%Earmark.Block.List{blocks: resources}] = resources

    resources =
      for %Earmark.Block.ListItem{blocks: [%Earmark.Block.Para{lines: [name]}]} <- resources do
        {title, _link} = parse_markdown_link(name)
        title
      end
  ...
  def sorted_entries(%Earmark.Block.List{blocks: entriesList}) do
    ...
    entries =
      Enum.map(entriesList, fn %Earmark.Block.ListItem{
                                 blocks: [%Earmark.Block.Para{lines: [line]}]
                               } ->
        line
      end)

    names =
      Enum.map(entries, fn line ->
  ...
  def collect_headings(
        [%Earmark.Block.Heading{content: heading, level: 2} | tail],
        found_headings,
        all_headings
      ) do
  ...
  def collect_headings(
        [%Earmark.Block.Heading{content: _heading, level: 1} | tail],
        found_headings,
        all_headings
      ) do
  ...
  def iterate_content([
        ...
        %Earmark.Block.Para{lines: _lines} | tail
      ]) do
    IO.puts("- #{heading}")
    iterate_content(tail)
  end
  ...
  def validate_list_item(%Earmark.Block.ListItem{
        blocks: [%Earmark.Block.Para{lines: [line]}],
        type: :ul
      }) do
    line =
